...
interface CircularMazeProps {
    userLocation: { posX: number; posY: number; facingX: number; facingY: number }
    rows: number
}

const CircularMaze = ({userLocation, rows}: CircularMazeProps) => {
    ...
    useEffect(() => {
        ...
        const renderMaze = () => {
            ...
            // Render user's position and facing direction
            let { posX, posY, facingX, facingY } = userLocation;
            facingY *= -1;
            ...
            ctx.lineTo(userX + facingX * 10, userY + facingY * 10);
            ctx.lineTo(userX + facingY * 5, userY - facingX * 5);
            ctx.lineTo(userX - facingY * 5, userY + facingX * 5);
            ctx.closePath();
            ctx.fill();
        };
        ...
        createMaze();
    }, [mazeContainerRef.current, canvasRef.current]);

    return (
        <div ref={mazeContainerRef} className="maze">
